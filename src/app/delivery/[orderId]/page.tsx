...
import { CheckCircle2, MapPin, Phone, Package, Navigation, Loader2, IndianRupee, ShieldCheck } from "lucide-react";
...
export default function RiderGatewayPage() {
  ...
  const { data: order, loading: orderLoading } = useDoc<Order>('orders', orderId);
  ...
  const handleMarkDelivered = () => {
    ...
    updateDoc(orderRef, { status: 'Completed' })
      ...
      .catch(() => {
        toast({ variant: 'destructive', title: "Error", description: "Could not sync delivery status." });
      })
      .finally(() => {
        setIsUpdating(false);
      });
  ...
  if (orderLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6 gap-4">
        <Loader2 className="h-8 w-8 animate-spin text-primary" />
        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Identifying Order Manifest...</p>
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6 text-center">
        ...
        <p className="text-xs font-bold text-muted-foreground uppercase mt-2">This order link may have expired.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f1f2f6] pb-12 flex flex-col items-center justify-center p-4">
        ...
        {!success ? (
          ...
          >
            ...
            <div className="flex flex-col items-center text-center space-y-2">
              ...
              <Badge variant="outline" className="font-black uppercase text-[10px] tracking-widest">Order: #{orderId.slice(-6).toUpperCase()}</Badge>
            </div>
              ...
              <CardContent className="p-8 space-y-8 text-left">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className="bg-green-100 text-green-700 text-[8px] font-black uppercase border-none">{order.paymentStatus === 'Success' ? 'PAID ONLINE' : 'CASH ON DELIVERY'}</Badge>
                  </div>
                ...
                <div className="bg-amber-50 border border-amber-100 p-6 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[9px] font-black text-amber-800 uppercase tracking-widest">Amount to Collect</p>
                      <p className="text-xl font-black text-amber-950">₹{order.paymentStatus === 'Success' ? '0.00' : order.total.toFixed(2)}</p>
                    </div>
                  </div>
            ...
            <button 
              ...
              className="text-[10px] font-black uppercase tracking-widest text-muted-foreground w-full text-center hover:text-[#333]"
            >
              Exit Terminal
            </button>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm font-bold text-muted-foreground uppercase max-w-xs">The outlet and customer have been notified of your delivery.</p>
            </div>
            <Button 
              variant="outline"

...
function ReviewsContent() {
  ...
  const { data: outlet, loading: outletLoading } = useDoc<Outlet>('outlets', outletId || 'dummy');
  const { data: userProfile } = useDoc<UserProfile>('users', user?.uid || 'dummy');
  const { data: reviews, loading: reviewsLoading } = useCollection<Review>(outletId ? `outlets/${outletId}/reviews` : 'dummy');
  ...
  if (!outletId) {
    return (
      <div className="flex flex-col min-h-screen bg-gray-50 items-center justify-center p-6 text-center">
        <h1 className="text-xl font-black uppercase">Store Reference Missing</h1>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f1f2f6] pb-24 pt-4">
